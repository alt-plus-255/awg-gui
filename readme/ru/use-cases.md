...
Резолвер **выключен**. В карточке пира укажите CIDR в **AllowedIPs (сервер → peer)** (например `192.168.1.13/32`). В клиентском `.conf` будет подсеть туннеля + эти CIDR, **без** `0.0.0.0/0`:
...
Через VPN — указанные ресурсы и подсеть AWG; обычный интернет и DNS — мимо туннеля (провайдер). Подробнее — [конфиги и пиры](configs-and-peers.md#allowedips-для-типа-сервер).
