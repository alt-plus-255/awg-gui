...
Резолвер **выключен**. В карточке пира укажите CIDR в **AllowedIPs (сервер → peer)** (например `192.168.1.13/32`). В клиентском `.conf` будет адрес интерфейса сервера + эти CIDR, **без** `0.0.0.0/0`:

```
AllowedIPs = 10.66.66.1/24, 192.168.1.13/32
```

Клиент ходит через VPN только к указанным ресурсам (и к адресу интерфейса), остальной трафик — мимо туннеля. Подробнее — [конфиги и пиры](configs-and-peers.md#allowedips-для-типа-сервер).
