...
Для **WG/AWG** панель разбирает `.conf` (`[Interface]` / `[Peer]`, включая параметры обфускации) и поднимает exit-туннель (`awgc…`) для sing-box. Версии AmneziaWG **несовместимы** между собой — укажите ту же, что на удалённом сервере (1.0 / 1.5 / 2.0).
