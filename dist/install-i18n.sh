...
_awg_gui_msg_en() {
  case "$1" in
    ...
    log_quiesce_exec_clients) printf '%s' 'Stopping app and panel-ops before AWG recreate (avoid docker exec races) ...' ;;
    ...
    err_upgrade_compose_timeout) printf '%s' 'compose up timed out during upgrade — check update.log; reboot and run awg-gui ensure-up if stuck' ;;
...
_awg_gui_msg_ru() {
  case "$1" in
    ...
    log_quiesce_exec_clients) printf '%s' 'Останавливаем app и panel-ops перед пересозданием AWG (избегаем гонок docker exec) ...' ;;
    ...
    err_upgrade_compose_timeout) printf '%s' 'compose up превысил таймаут при обновлении — см. update.log; при зависании перезагрузите сервер и выполните awg-gui ensure-up' ;;
