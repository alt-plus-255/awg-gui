...
_awg_gui_msg_en() {
  case "$1" in
    ...
    log_quiesce_exec_clients) printf '%s' 'Stopping app, panel-ops and caddy before AWG recreate (avoid docker exec races) ...' ;;
    ...
    log_upgrade_compose_phase) printf '%s' 'Compose up (%s) ...' "$1" ;;
    log_upgrade_compose_phase_base) printf '%s' 'Compose up (db + docker-proxy) ...' ;;
    log_upgrade_compose_phase_awg) printf '%s' 'Compose up (awg) ...' ;;
    log_upgrade_compose_phase_front) printf '%s' 'Compose up (app + panel-ops + caddy) ...' ;;
    err_force_remove_timeout) printf '%s' 'Timed out removing container %s — reboot may be required' "$1" ;;
    warn_docker_restart_required) printf '%s' 'Container process gone but Docker still tracks it — restarting Docker engine ...' ;;
    err_upgrade_compose_timeout) printf '%s' 'compose up timed out at phase %s — check update.log; reboot and run awg-gui ensure-up if stuck' "$1" ;;
    err_upgrade_compose_failed) printf '%s' 'compose up failed at phase %s (see output above)' "$1" ;;
...
_awg_gui_msg_ru() {
  case "$1" in
    ...
    log_quiesce_exec_clients) printf '%s' 'Останавливаем app, panel-ops и caddy перед пересозданием AWG (избегаем гонок docker exec) ...' ;;
    log_upgrade_phase_quiesce) printf '%s' 'Фаза обновления: остановка exec-клиентов ...' ;;
    log_upgrade_phase_kernel) printf '%s' 'Фаза обновления: подготовка kernel на хосте ...' ;;
    log_upgrade_phase_awg_remove) printf '%s' 'Фаза обновления: удаление awggui-awg ...' ;;
    log_upgrade_phase_compose) printf '%s' 'Фаза обновления: compose up (поэтапно) ...' ;;
    log_upgrade_compose_phase) printf '%s' 'Compose up (%s) ...' "$1" ;;
    log_upgrade_compose_phase_base) printf '%s' 'Compose up (db + docker-proxy) ...' ;;
    log_upgrade_compose_phase_awg) printf '%s' 'Compose up (awg) ...' ;;
    log_upgrade_compose_phase_front) printf '%s' 'Compose up (app + panel-ops + caddy) ...' ;;
    err_force_remove_timeout) printf '%s' 'Таймаут удаления контейнера %s — может потребоваться перезагрузка' "$1" ;;
    warn_docker_restart_required) printf '%s' 'Процесс контейнера уже завершён, но Docker всё ещё его отслеживает — перезапуск Docker ...' ;;
    err_upgrade_compose_timeout) printf '%s' 'compose up превысил таймаут на фазе %s — см. update.log; при зависании перезагрузите сервер и выполните awg-gui ensure-up' "$1" ;;
    err_upgrade_compose_failed) printf '%s' 'compose up не удался на фазе %s (см. вывод выше)' "$1" ;;
