...
_awg_gui_msg_en() {
  case "$1" in
    ...
    log_upgrade_compose_phase) printf '%s' 'Compose up (%s) ...' ;;
    ...
    err_force_remove_timeout) printf '%s' 'Timed out removing container %s — reboot may be required' ;;
    warn_docker_restart_required) printf '%s' 'Container process gone but Docker still tracks it — restarting Docker engine ...' ;;
    err_upgrade_compose_timeout) printf '%s' 'compose up timed out at phase %s — check update.log; reboot and run awg-gui ensure-up if stuck' ;;
    err_upgrade_compose_failed) printf '%s' 'compose up failed at phase %s (see output above)' ;;
...
_awg_gui_msg_ru() {
  case "$1" in
    ...
    log_upgrade_compose_phase_awg) printf '%s' 'Compose up (awg) ...' ;;
    log_upgrade_compose_phase_front) printf '%s' 'Compose up (app + panel-ops + caddy) ...' ;;
    err_force_remove_timeout) printf '%s' 'Таймаут удаления контейнера %s — может потребоваться перезагрузка' ;;
    ...
    err_upgrade_compose_timeout) printf '%s' 'compose up превысил таймаут на фазе %s — см. update.log; при зависании перезагрузите сервер и выполните awg-gui ensure-up' ;;
    err_upgrade_compose_failed) printf '%s' 'compose up не удался на фазе %s (см. вывод выше)' ;;
