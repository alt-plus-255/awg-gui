...
VPN-сервер AmneziaWG (версии протокола **1.0**, **1.5**, **2.0** и **3.1**, по умолчанию **3.1**) с Go API и админ-панелью на Quasar Vue. Все сервисы работают в Docker-контейнерах с префиксом `awggui`.
...
До **20** конфигов AmneziaWG (UDP **51820–51839**): у каждого свой интерфейс, подсеть, порт и **версия протокола** (**1.0** / **1.5** / **2.0** / **3.1** — у разных конфигов версии могут отличаться). Типы **Сервер** (VPN в интернет) и **Виртуальная сеть** (изолированная LAN).

→ [Подробнее: конфиги и пиры](readme/ru/configs-and-peers.md)

### Версии протокола AmneziaWG

При создании или редактировании конфига выбирается версия **1.0**, **1.5**, **2.0** или **3.1** (по умолчанию — последняя). Версии **несовместимы** между собой. При смене версии панель предупреждает и **перегенерирует** параметры обфускации и CPS (`I1`–`I5`); после сохранения peer нужно заново скачать `.conf` / QR / `vpn://`. Для **1.5+** доступна автогенерация маскировочных пакетов CPS (шаблоны QUIC, DNS, STUN, SIP, DTLS, RTP, random) с проверкой синтаксиса и размеров.
...
Для типа **Сервер** без резолвера: если у пира указаны CIDR в AllowedIPs, в клиентском `.conf` будет подсеть туннеля (`10.66.66.0/24`) + эти CIDR (split-tunnel) вместо полного туннеля; интернет мимо VPN. При включении резолвера — подтверждение и переход на `0.0.0.0/0`.
